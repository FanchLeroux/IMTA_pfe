...
import numpy as np
from doe.tools import discretization, softDiscretization
...
#                      doe_size {tupple (1x2)}[pixel] : size of the DOE 
...
#                      compute_efficiency {bool} : If 1, efficiency is computed and 
...
#8<---------------------------------------------------------------------------------------------
    ...
    amp_image = np.zeros(doe_size)                                                                             
    amp_image[doe_size[0]//2-target_size[0]//2:doe_size[0]//2-     # Amplitude output field = 0
    ...
    for k in range(n_iter):
        ...
        field_image = np.fft.fftshift(np.fft.fft2(field_DOE))           # field image = TF field DOE
        phase_image = np.angle(field_image)                             # save image phase
        amp_image[doe_size[0]//2-target_size[0]//2:doe_size[0]//2-
                  target_size[0]//2+target_size[0], 
                  doe_size[1]//2-target_size[1]//2:doe_size[1]//2-
                  target_size[1]//2+target_size[1]] = rfact*target_amp  # force the amplitude of the DOE to the target 
    ...
    if n_levels != 0:
        ...
        for k in range(n_iter):
            ...
            field_image = np.fft.fftshift(np.fft.fft2(field_DOE))      # image = TF du DOE
            phase_image = np.angle(field_image)                        # save image phase
            amp_image[doe_size[0]//2-target_size[0]//2:doe_size[0]//2- # force the amplitude of the DOE to the target
                      target_size[0]//2+target_size[0],                # amplitude inside the ROI. Outside the ROI, the 
                      doe_size[1]//2-target_size[1]//2:doe_size[1]//2- # amplitude is free
    ...
    if compute_efficiency:
        ...
        recovery = np.absolute(np.fft.fftshift(np.fft.fft2(field_DOE)))**2 # Final image = |TF field DOE|^2
        roi = recovery[doe_size[0]//2-target_size[0]//2:doe_size[0]//2-
                       target_size[0]//2+target_size[0],                   # efficiency = energy inside target zone / total energy
                  doe_size[1]//2-target_size[1]//2:doe_size[1]//2-
                  target_size[1]//2+target_size[1]]
        efficiency = np.sum(roi[target!=0])/np.sum(recovery)
